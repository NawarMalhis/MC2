...
class C3F2(torch.nn.Module):
    def __init__(self, in_sz=510, in_chl=7, chl_inc=6, c1k_sz=7, c2k_sz=7, c3k_sz=7, pk_sz=7, p_stp=1, fc1_out=150,
        ...
        p1o_sz = int((c1o_sz - pk1_sz) / p_stp + 1)
        c2o_sz = p1o_sz - c2k_sz + 1
        p2o_sz = int((c2o_sz - pk_sz) / p_stp + 1)
        ...
        self.pool1 = torch.nn.AvgPool1d(kernel_size=pk1_sz, stride=p_stp)
        self.conv2 = torch.nn.Conv1d(in_channels=c1o_chl, out_channels=c2o_chl, kernel_size=c2k_sz)
        self.pool2 = torch.nn.AvgPool1d(kernel_size=pk_sz, stride=p_stp)
